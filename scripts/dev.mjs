...
const files=['apps/sample-reference-service/server.ts','apps/platform-api/server.ts','apps/platform-worker/server.ts'];
const children=files.map(file=>spawn(process.execPath,['--import','tsx',file],{env:file==='apps/platform-worker/server.ts'?env:publicProcessEnv,stdio:'inherit',windowsHide:true}));
