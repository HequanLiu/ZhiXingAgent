...
import {NodeAcceptance,type AcceptanceTarget} from './NodeAcceptance';
import {AnalysisPanel,type Analysis} from './AnalysisPanel';
import {useEffect,useRef,useState} from 'react';
import {Alert,Button,Dropdown,Empty,Form,Input,InputNumber,Progress,Select,Spin,Table,Tag,message} from 'antd';
...
export function App({identity,onLogout}:{identity?:{tenant:string;actor:string};onLogout?:()=>void}){
  ...
  return <MembersContext.Provider value={membersValue}><div className={mobile?'app mobile':'app'}>{contextHolder}
    ...
    <BusinessModal title="声研助理分析与业务证据" open={analysisOpen} onCancel={()=>setAnalysisOpen(false)} footer={null} width={760}><AnalysisPanel loadEvidence={id=>api(`/api/analysis/${id}/evidence`)} pending={retry.isPending} onRetry={isManager?item=>retry.mutate(item):undefined} items={(analysisQuery.data??[]).filter(item=>item.object_id===order?.id)} error={analysisQuery.isError} versions={Object.fromEntries(orders.map(o=>[o.id,o.version]))}/></BusinessModal>
    <BusinessModal title={`${order?.id??''} · 反馈记录与交期依据`} description="查看每一次进度反馈、验收结论与交期评估的来源。" open={evidenceOpen} onCancel={()=>setEvidenceOpen(false)} footer={null} width={880}>{order&&<OrderEvidence order={order}/>}</BusinessModal>
    ...
    <BusinessModal title={`设置负责人 · ${assignNode?.name??''}`} open={!!assignNode} onCancel={()=>setAssignNode(null)} confirmLoading={mutation.isPending} onOk={()=>mutation.mutate({path:`/api/soundlab/orders/${order?.id}/assign`,body:{nodeId:assignNode?.id,owner:newOwner,expectedVersion:editVersion}})}><p>调整节点负责人，后续由该负责人提交反馈。</p><Select style={{width:'100%'}} aria-label="节点负责人" value={newOwner} onChange={setNewOwner} options={ownerOptions}/></BusinessModal>
