...
export function NodeAcceptance({target,pending,onCancel,onSubmit}:{target:AcceptanceTarget;pending:boolean;onCancel:()=>void;onSubmit:(note:string,decision:'accept'|'return')=>void}) {
 const {people,ownerOptions,isManager}=useMembers();
  ...
  return <BusinessModal open title={`验收节点 · ${target.node.name}`} description="核对完成情况与验收标准，再选择通过或退回。" onCancel={onCancel} confirmLoading={pending} okText={returning?'确认退回返工':'确认验收通过'} okButtonProps={{danger:returning,disabled:returning&&!note.trim()}} onOk={()=>onSubmit(note,decision)}>
