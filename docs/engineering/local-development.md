...
以下保留开发配置、历史验证记录和集成限制；项目概览见[根 README](../../README.md)。

## 本地启动

要求 Node 22.19+、pnpm 11.7.0、Docker Desktop。以下命令均在项目根目录执行。
...
企业微信协议与队列已有本地加密回调、数据库和模拟传输验证；实际企业连接仍需应用凭据、成员 UserID 映射及公网 HTTPS 回调地址，见 [企微接入](wecom-setup.md)。本轮没有发送真实企微消息或新增真实模型请求。当前运行保持模型分析与企微外发关闭。

部署、迁移、恢复、健康检查、发布包见 [部署手册](deployment-runbook.md)。`pnpm smoke:browser` 通过独立 Chrome 执行桌面模板/批量导入和移动反馈；`pnpm release:package` 输出含校验清单的代码包，不打包密钥。CI 已提供，远端执行及正式服务器部署需要实际环境。模型用量无回执时保持未知，不推算费用。

最新测试结果和浏览器证据以 [验收记录](soundlab-acceptance.md) 为准；完整范围见 [交付清单](../superpowers/plans/2026-09-07-soundlab-full-delivery.md)。业务写入使用固定接口，不透传任意业务 URL。
